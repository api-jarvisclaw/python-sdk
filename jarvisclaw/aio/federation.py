"""AsyncFederationClient — federation registry, API catalogue, and peer management.

The sync FederationClient had no async counterpart, so an async application had to drop to raw
httpx for every federation call or block its event loop on the sync client.

Lives in its own module rather than being appended to client.py: that file is already 900 lines
of eight clients, and federation is the one surface with three distinct auth tiers worth
...
class AsyncFederationClient(AsyncBaseClient):
    """Async client for federation endpoints.
    ...
    Usage:
        ...
        async with FederationClient(api_key="sk-...") as fed:
    ...
    async def invoke(
        ...
        *,
        ...
        so a caller needs to know nothing about the federation subsystem — that decoupling is
